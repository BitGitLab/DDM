...
//		System.out.println("schedule??????????:"+task.schedule);
	...
	public Vector<String> generateStopPatternSchedule(StringBuilder CON_PREVENT)
	{
		id.incrementAndGet();
		ConstraintsSolving task = new ConstraintsSolving(config, id.get());
		declareVariables(CONS_ASSERT.append(CON_PREVENT));


		StringBuilder msg = new StringBuilder(CONS_SETLOGIC).append(CONS_DECLARE).append(CONS_ASSERT).append(CON_PREVENT).append(CONS_GETMODEL);
