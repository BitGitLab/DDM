...
//    @test
    ...
    public void testFullAllocationAndFree() throws Exception {
    		allocateAndFreeAndEnsureNoErrors(100, 2, 2);
        //allocateAndFreeAndEnsureNoErrors(1, 2, 2);
    }

    private AllocationVector vector = new AllocationVector(1);
    ...
    private void checkForErrors(int[][] threadResults) {
        for (int[] threadResult : threadResults) {
            if (threadResult[0] == -2 || threadResult[0] == -3) {
                throw new RuntimeException("Tried to allocate a block which was already allocated or free a block that was already free");
